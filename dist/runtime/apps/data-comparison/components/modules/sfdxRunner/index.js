...
- Linux: /usr/local/bin/sf
    ...
    async runWithJson(command) {
        // Smart flag detection: GraphQL commands don't support --json, others do
        const isGraphQLCommand = command.includes('api request graphql');
        
        let finalCommand = command;
        if (!isGraphQLCommand && !command.includes('--json')) {
            // Add --json for non-GraphQL commands that need structured output
