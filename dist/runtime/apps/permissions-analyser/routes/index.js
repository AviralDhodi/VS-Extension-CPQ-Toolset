...
router.get('/components/:component/:file', (req, res, next) => {
  ...
  // Use enhanced version for config generator
  const isEnhanced = component === 'configGenerator' && file.startsWith('enhanced-');
  const fileName = isEnhanced ? file : file;
...
// Static assets route removed - using dynamic serving above for pkg compatibility
...
router.post("/api/permissions/metadata", async (req, res) => {
    try {
        ...
        const { METADATA_DEFINITIONS } = require('../utils/metadataDefinitions');
        
        logger.info("Getting permission metadata...");
        ...
        for (const org of orgs) {
            ...
            for (const permType of permissionTypes) {
                ...
                if (metadataResult.success) {
                    ...
                    // Store metadata without assignment counts
                    results[org][permType] = metadata;
                }
            }
        }
