...
class FetchCoordinator {
  ...
  _startFetcher(task, callback) {
    ...
    // Ensure output directory exists
    if (!pkgReader.existsSync(task.outputDir)) {
      pkgReader.mkdirSync(task.outputDir, { recursive: true })
    }

    // Use fork for worker processes
  ...
  _ensureOutputDirectory(outputDir) {
    if (!pkgReader.existsSync(outputDir)) {
      pkgReader.mkdirSync(outputDir, { recursive: true })
