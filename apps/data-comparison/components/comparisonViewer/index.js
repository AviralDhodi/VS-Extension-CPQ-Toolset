...
class ComparisonViewer {
    ...
    renderDiffView() {
        ...
        if (filtered.length === 0) {
            diffContent.innerHTML = `
                <div class="empty-state slds-text-align_center slds-p-around_x-large">
                    ...
                    <p class="slds-text-body_regular">No comparison results match the current filter.</p>
                </div>
            `;
            return;
        }
        ...
        Object.keys(grouped).sort().forEach(object => {
            html += `<div class="diff-object slds-m-bottom_large">
                <div class="diff-object-header slds-p-around_small">
                    <h4 class="diff-object-title slds-text-heading_small">${object}</h4>
                </div>
