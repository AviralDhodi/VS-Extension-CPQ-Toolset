...
class PythonRunner {
  ...
  detectPython() {
    ...
    if (platform === 'win32') {
      try {
        // Look for embedded Python in multiple possible locations
        // PathResolver now gives us the correct real filesystem path
        const possiblePythonPaths = [
          path.join(this.pathResolver.extensionRoot, 'runtime', 'py', 'python.exe'),
          path.join(this.pathResolver.extensionRoot, 'py', 'python.exe')
        ];
        
        for (const embeddedPythonPath of possiblePythonPaths) {
        ...
        logger.warn(`Embedded Python not found in any of the expected locations`);
      } catch (error) {
  ...
  async runMultiOrgComparison(comparisonId, configPath, outputPath, progressCallback) {
    const scriptPath = this.pathResolver.getPythonScript('data-comparison', 'multi_org_comparison_optimized.py');
    ...
    try {
      ...
      // The output should be in comparison_results subdirectory of dataDir
      const comparisonOutputDir = path.join(dataDir, 'comparison_results');
      ...
      const result = await this.runScriptFile(scriptPath, [
        ...
        '--output-dir', comparisonOutputDir,
        '--chunk-size', '50000'
