  ...
  async getObjects(targetOrg, options = {}) {
    try {
      ...
      if (Array.isArray(objects) && objects.length > 0 && typeof objects[0] === 'string') {
        logger.info(`[SFDX-${targetOrg}] Using SF CLI string format, raw count: ${objects.length}`);
        // SF CLI returns array of strings
        const beforeFilter = objects.length;
        objects = objects
          .filter(name => name && !name.startsWith('__'))
        logger.info(`[SFDX-${targetOrg}] After filtering __ objects: ${objects.length} (removed ${beforeFilter - objects.length})`);
        ...
        if (uniqueNames.size !== objects.length) {
          logger.warn(`[SFDX-${targetOrg}] FOUND DUPLICATES: ${objects.length} objects but only ${uniqueNames.size} unique names`);
          const duplicates = objects.filter((obj, index, arr) => arr.findIndex(o => o.name === obj.name) !== index);
          logger.warn(`[SFDX-${targetOrg}] Duplicate object names:`, duplicates.map(d => d.name));
        }
      } else {
        logger.info(`[SFDX-${targetOrg}] Using legacy object format, raw count: ${objects.length}`);
