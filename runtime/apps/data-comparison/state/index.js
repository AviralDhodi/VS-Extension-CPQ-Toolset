...
class DataComparisonStateManager {
    ...
    setState(component, data = {}) {
        console.log(`[StateManager] [Processing] Setting state: ${this.currentState.component} -> ${component}`);
        ...
        console.log(`[StateManager] [Success] State updated:`, this.currentState);
    ...
    shouldUseImproved(component) {
        // For v3, always use improved version for configGenerator and comparisonViewer
        return component === 'configGenerator' || component === 'comparisonViewer';
    }
    ...
    async generateAppView(component, data = {}) {
        ...
        console.log(`[StateManager] [List] Component exists: ${pkgReader.existsSync(componentPath)}`);
        
        // Read component files - use improved version for configGenerator and SLDS for comparisonViewer
        const useImproved = component === 'configGenerator';
        const useSLDS = component === 'comparisonViewer';
        
        let htmlPath, cssPath, jsPath;
        
        if (useImproved) {
            htmlPath = path.join(componentPath, 'improved-index.html');
            cssPath = path.join(componentPath, 'index.css'); // improved-index.css doesn't exist
            jsPath = path.join(componentPath, 'improved-index.js');
        } else if (useSLDS) {
            htmlPath = path.join(componentPath, 'slds-index.html');
            cssPath = path.join(componentPath, 'slds-index.css');
            jsPath = path.join(componentPath, 'index.js'); // Use original JS
        } else {
            htmlPath = path.join(componentPath, 'index.html');
            cssPath = path.join(componentPath, 'index.css');
            jsPath = path.join(componentPath, 'index.js');
        }
        
        console.log(`[StateManager] 📄 HTML: ${htmlPath} - exists: ${pkgReader.existsSync(htmlPath)}`);
        console.log(`[StateManager] 🎨 CSS: ${cssPath} - exists: ${pkgReader.existsSync(cssPath)}`);
        console.log(`[StateManager] ⚡ JS: ${jsPath} - exists: ${pkgReader.existsSync(jsPath)}`);
        ...
        if (pkgReader.existsSync(htmlPath)) {
            const fullHtml = pkgReader.readFileSync(htmlPath, 'utf8');
            console.log(`[StateManager] 📖 Read HTML file, length: ${fullHtml.length}`);
            
            const bodyMatch = fullHtml.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
            if (bodyMatch) {
                componentHtml = bodyMatch[1];
                console.log(`[StateManager] [Extract] Extracted body content, length: ${componentHtml.length}`);
            } else {
            ...
            `;
            console.log(`[StateManager] [Warning] HTML file not found, using placeholder`);
        }
        
        // Read component CSS
        if (pkgReader.existsSync(cssPath)) {
            componentCss = pkgReader.readFileSync(cssPath, 'utf8');
            console.log(`[StateManager] 🎨 Read CSS file, length: ${componentCss.length}`);
        } else {
            console.log(`[StateManager] [Warning] CSS file not found`);
        }
        
        // Read component JS
        if (pkgReader.existsSync(jsPath)) {
            componentJs = pkgReader.readFileSync(jsPath, 'utf8');
            ...
            console.log(`[StateManager] [Warning] JS file not found`);
        }
...
<body data-component="${component}" data-app="data-comparison" data-theme="light" id="app-body">
    ...
    <script>
        console.log('[AppView] [Starting] Component ${component} script executing');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            console.log('[AppView] [List] DOMContentLoaded for component: ${component}');
            ...
            if ('${component}' === 'comparisonViewer') {
                try {
                    ...
                    console.log('[AppView] [Success] ComparisonViewer initialized successfully');
                } catch (error) {
        ...
        console.log(`[StateManager] [Success] Generated complete appView HTML, total length: ${appViewHtml.length}`);
        ...
        return `
            ...
            function setupShellCommunication(componentName) {
                ...
                window.changeState = function(newComponent, data = {}) {
                    console.log('[ShellComm] [Processing] Changing state to:', newComponent);
                ...
                console.log('[ShellComm] [Success] Shell communication setup complete');
            }
            ...
            function applyTheme(theme, variables) {
                ...
                if (variables) {
                    ...
                    console.log(\`[ShellComm] [Success] Applied \${appliedCount} theme variables\`);
                } else {
                    console.log('[ShellComm] [Warning] No theme variables provided, using CSS defaults');
                }
...
console.log('[StateManager] [Building] Creating DataComparisonStateManager instance');
