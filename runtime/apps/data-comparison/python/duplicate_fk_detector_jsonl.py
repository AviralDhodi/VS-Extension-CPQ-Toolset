    ...
    def detect_all_duplicates(self):
        ...
        for item in self.comparison_dir.iterdir():
            if item.is_dir() and not item.name.startswith('.'):
                ...
                # v3.1.18 - Only add org to duplicates if it actually has duplicates
                if org_duplicates and any(len(obj_dups) > 0 for obj_dups in org_duplicates.values()):
...
def main():
    ...
    if summary['total_duplicate_fks'] > 0:
        print(f"\nWARNING: Duplicate foreign keys detected! Review the report at: {report_path}")
        print("\nRecommendations:")
