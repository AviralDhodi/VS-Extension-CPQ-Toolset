...
class OptimizedSalesforceDataComparator:
    ...
    def __init__(self, chunk_size: int = 50000, exclude_fields: List[str] = None):
        ...
        self.folder_to_display = {}
        self.display_to_folder = {}
        self.folder_to_username = {}  # New mapping for folder to username
        self.final_differences_df = []
        self.blacklisted_fks = set()  # Set to store blacklisted foreign keys
        self.configured_fields = {}  # Dict to store configured fields per object
        
        # Default exclusion list for common Salesforce system fields and metadata
        default_exclusions = [
            'CreatedDate', 'CreatedBy', 'CreatedById', 'CreatedBy_Name',
            'LastModifiedDate', 'LastModifiedBy', 'LastModifiedById', 'LastModifiedBy_Name',
            'SystemModstamp', 'Id',
            # Exclude metadata fields added during data extraction
            '_sourceOrg', '_fetchTimestamp', '_recordIndex', '_primaryKey', '_objectName'
        ]
    ...
    def discover_orgs_and_objects(self, base_path: str) -> Dict:
        ...
        # Build mapping of folder names to display names
        # This handles both username-based folders and alias-based folders
        folder_to_display = {}
        display_to_folder = {}
        
        for folder in org_folders:
            # Check if this folder matches a known alias or username pattern
            display_name = None
            username = None
            
            # First check if it's already an alias
            for mapped_folder, alias in self.org_display_names.items():
                if folder == alias or folder == mapped_folder:
                    display_name = alias
                    # Get username for this folder
                    username = self.folder_to_username.get(folder, None) or self.folder_to_username.get(mapped_folder, None)
                    break
            
            # If not found, use the folder name as-is
            if not display_name:
                display_name = folder
                # Try to reconstruct username from folder name
                if '_' in folder and not folder.startswith('.'):
                    # Likely a username folder like john_doe_company_com
                    username = folder.replace('_', '@', 1).replace('_', '.')
                else:
                    username = folder
            
            folder_to_display[folder] = display_name
            display_to_folder[display_name] = folder
            if username:
                self.folder_to_username[folder] = username
            self.logger.info(f"Mapped folder '{folder}' to display name '{display_name}' with username '{username}'")
        
        # Update the discovered orgs with display names
        self.discovered_orgs = sorted(list(folder_to_display.values()))
        self.folder_to_display = folder_to_display
        self.display_to_folder = display_to_folder
        
        # Find common objects across all orgs
        all_org_objects = {}
        for folder in org_folders:
            org_path = os.path.join(base_path, folder)
            ...
            display_name = folder_to_display[folder]
            all_org_objects[display_name] = set(objects)
        ...
        self.logger.info(f"Discovered orgs: {self.discovered_orgs}")
    ...
    def _load_foreign_key_config(self, base_path: str) -> bool:
        ...
        try:
            ...
            # Extract foreign key mappings and field lists from objects dictionary
            ...
            for obj_name, obj_config in objects_config.items():
                if isinstance(obj_config, dict):
                    ...
                    if 'fields' in obj_config:
                        ...
                        # Always include the foreign key field in comparison
                        if 'foreignKey' in obj_config and obj_config['foreignKey'] not in fields:
                            fields.append(obj_config['foreignKey'])
                        self.configured_fields[obj_name] = fields
                        self.logger.info(f"Found {len(fields)} configured fields for {obj_name}")
            ...
            for org_config in orgs_config:
                if isinstance(org_config, dict):
                    ...
                    # Map both username-based and alias-based folder names
                    username_folder = username.replace('@', '_').replace('.', '_')
                    if username_folder:
                        self.org_display_names[username_folder] = alias
                        self.folder_to_username[username_folder] = username  # Store username mapping
                    # Also map the alias directly
                    self.org_display_names[alias] = alias
                    self.folder_to_username[alias] = username  # Store username for alias too
                elif isinstance(org_config, str):
                    # Handle legacy format where orgs is just array of aliases
                    self.org_display_names[org_config] = org_config
                    self.folder_to_username[org_config] = org_config  # Use as-is for legacy
            
            self.logger.info(f"Extracted {len(self.foreign_key_mappings)} foreign key mappings from config")
            self.logger.info(f"Configured field counts by object: {[(obj, len(fields)) for obj, fields in self.configured_fields.items()]}")
            return True
    ...
    def _load_sf_object_data(self, base_path: str, org: str, sf_object: str, key_field: str) -> Optional[pd.DataFrame]:
        ...
        # Get the actual folder name for this org
        folder_name = self.display_to_folder.get(org, org)
        
        # Try parquet first (fastest), then JSONL with parquet caching, then CSV
        parquet_file = os.path.join(base_path, folder_name, f"{sf_object}.parquet")
        jsonl_file = os.path.join(base_path, folder_name, f"{sf_object}.jsonl")
        csv_file = os.path.join(base_path, folder_name, f"{sf_object}.csv")
    ...
    def _create_composite_key(self, row: pd.Series, primary_key: str) -> Tuple:
        ...
        # Process each column value to ensure it's hashable
        for col in data_cols:
            ...
            if value is None or pd.isna(value):
                ...
                # Convert complex types to JSON string for hashing
                key_data.append(json.dumps(value, sort_keys=True))
            elif isinstance(value, (int, float, str, bool)):
                # Simple types can be used directly
                key_data.append(value)
            else:
                # Convert any other type to string
                key_data.append(str(value))
    ...
    def _records_identical(self, record1: pd.Series, record2: pd.Series) -> bool:
        ...
        exclude_cols = {'org_name', 'composite_key', 'object_name', 'primary_key'}
        object_name = record1.get('object_name', '')
        
        # Get configured fields for this object
        configured_fields = self.configured_fields.get(object_name, [])
        
        # If no fields configured, compare all non-metadata fields
        if configured_fields:
            # Filter out excluded fields from configured fields
            fields_to_compare = [f for f in configured_fields if f not in self.exclude_fields]
        else:
            # Fallback to all fields except metadata and excluded fields
            metadata_fields = {'_sourceOrg', '_fetchTimestamp', '_recordIndex', '_primaryKey', '_objectName'}
            all_excluded = exclude_cols.union(metadata_fields).union(self.exclude_fields)
            fields_to_compare = [f for f in record1.index if f not in all_excluded]
        
        for col in fields_to_compare:
            if col not in exclude_cols and col in record1.index and col in record2.index:
    ...
    def _find_and_record_field_differences(self, ref_record: pd.Series, 
        ...
        if configured_fields:
            ...
            metadata_fields = {'_sourceOrg', '_fetchTimestamp', '_recordIndex', '_primaryKey', '_objectName'}
            all_excluded = exclude_cols.union(metadata_fields).union(self.exclude_fields)
            fields_to_compare = [f for f in ref_record.index if f not in all_excluded]
        
        for field_name in fields_to_compare:
            if field_name not in exclude_cols and field_name in ref_record.index and field_name in comp_record.index:
    ...
    def _record_missing_difference(self, ref_record: pd.Series, ref_org: str,
        ...
        primary_key_value = ref_record['primary_key']
        foreign_key_field = self._get_foreign_key_field(object_name)
        
        # For missing records, create a single entry showing the record is missing
        # NOT an entry for each field!
        diff_record = {
            'ForeignKeyField': foreign_key_field,
            'ForeignKeyValue': primary_key_value,
            'ObjectFieldName': object_name,  # Just the object name, not individual fields
            'DifferenceType': 'RECORD_MISSING',
            f'Org_{self.folder_to_username.get(ref_org, ref_org)}': 'EXISTS',
            f'Org_{self.folder_to_username.get(comp_org, comp_org)}': 'MISSING'
        }
    ...
    def _record_field_difference(self, primary_key_value: Any, object_name: str,
        ...
        # This method should only be called for field value differences
        # Missing records are handled by _record_missing_difference
        ...
        def clean_value(val):
            if pd.isna(val) or val is None:
                return 'MISSING'
            # Convert to string and preserve the full content
            return str(val)
        ...
        diff_record = {
            ...
            f'Org_{self.folder_to_username.get(org1, org1)}': clean_value(val1),
            f'Org_{self.folder_to_username.get(org2, org2)}': clean_value(val2)
        }
    ...
    def _create_all_org_view(self, pairwise_df: pd.DataFrame) -> pd.DataFrame:
        """Transform pairwise comparisons into comprehensive ALL_ORG view"""
        # Group by key fields to consolidate all org values
        key_fields = ['ForeignKeyField', 'ForeignKeyValue', 'ObjectFieldName', 'DifferenceType']
        
        # Get all unique records based on key fields
        unique_records = pairwise_df[key_fields].drop_duplicates()
        
        # Initialize result list
        all_org_records = []
        
        # Get all org columns from pairwise comparisons (using display names)
        org_columns = [col for col in pairwise_df.columns if col.startswith('Org_')]
        all_orgs = sorted(list(set([col.replace('Org_', '') for col in org_columns])))
        ...
        for _, record in unique_records.iterrows():
            ...
            # Find all matching rows in pairwise df
            mask = True
            for field in key_fields:
                mask = mask & (pairwise_df[field] == record[field])
            
            matching_rows = pairwise_df[mask]
            
            # Collect values from all orgs
            for org in all_orgs:
                org_col = f'Org_{org}'
                # Get value from any matching row that has this org
                value = 'MISSING'
                for _, row in matching_rows.iterrows():
                    if org_col in row and pd.notna(row[org_col]):
                        value = row[org_col]
                        break
                new_record[org_col] = value
            
            all_org_records.append(new_record)
        
        # Create final dataframe with all orgs
        result_df = pd.DataFrame(all_org_records)
        
        # Reorder columns: key fields first, then sorted org columns
        final_cols = key_fields + [f'Org_{org}' for org in all_orgs]
    ...
    def _generate_output_files(self, output_dir: str) -> Dict:
        ...
        if not self.final_differences_df:
            ...
            summary = {
                ...
                'total_differences': 0,
                'organizations': [self.folder_to_display.get(org, org) for org in self.discovered_orgs],
            ...
            # Transform pairwise comparisons into ALL_ORG view
            all_org_df = self._create_all_org_view(differences_df)
            
            # Main differences file
            main_output = os.path.join(output_dir, 'all_differences.csv')
            # Use explicit quoting for all non-numeric values to handle special characters
            all_org_df.to_csv(main_output, index=False, quoting=1)  # csv.QUOTE_ALL
...
def main():
    ...
    # Use the provided output-dir if specified, otherwise use comparison_results in the base_path
    if args.output_dir:
