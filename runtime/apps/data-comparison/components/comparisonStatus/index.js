...
class ComparisonStatusManager {
    ...
    async initializeComparison() {
        ...
        try {
            ...
            // Load initial status to get config
            await this.getComparisonStatus();
    ...
    updatePhases(phases) {
        ...
        Object.keys(phases).forEach(phaseKey => {
            ...
            if (phaseEl) {
                // Update phase status class for SLDS Path
                phaseEl.classList.remove('slds-is-complete', 'slds-is-current', 'slds-is-active', 'slds-is-incomplete', 'slds-has-error');
                
                if (phase.status === 'completed') {
                    phaseEl.classList.add('slds-is-complete');
                } else if (phase.status === 'in_progress') {
                    phaseEl.classList.add('slds-is-current', 'slds-is-active');
                } else if (phase.status === 'failed') {
                    phaseEl.classList.add('slds-has-error');
                } else {
                    phaseEl.classList.add('slds-is-incomplete');
                }
        ...
        switch (phaseKey) {
            case 'dataFetch':
                ...
                if (data.config && data.config.orgs) {
                    ...
                    const orgNames = data.config.orgs.map(org => 
                        typeof org === 'string' ? org : (org.username || org.alias || '[Unknown]')
                    );
                    details += `<li>Organizations: ${orgNames.join(', ')}</li>`;
                }
                break;
    ...
    displayWarnings(warnings) {
        ...
        if (!warningSection) {
            ...
            warningSection.className = 'slds-m-top_medium';
            
            // Insert after progress bar
            const progressContainer = document.querySelector('.slds-progress-bar').parentNode;
            if (progressContainer && progressContainer.parentNode) {
                progressContainer.parentNode.insertBefore(warningSection, progressContainer.nextSibling);
            }
        }
        ...
        warnings.forEach(warning => {
            if (warning.type === 'duplicate_foreign_keys' && warning.severity === 'high') {
                ...
                warningDiv.className = 'slds-scoped-notification slds-media slds-scoped-notification_dark slds-m-bottom_medium';
                warningDiv.setAttribute('role', 'alert');
                warningDiv.innerHTML = `
                    <div class="slds-media__figure">
                        <span class="slds-icon_container slds-icon-utility-warning slds-icon_container_warning" title="Warning">
                            <svg class="slds-icon slds-icon_small" aria-hidden="true">
                    ...
                    <div class="slds-media__body">
                        ...
                        <div class="slds-scoped-notification__body">
                            <p>${warning.message}</p>
    ...
    formatDuplicateDetails(details) {
        ...
        html += '</ul>';
        html += '<p class="slds-text-body_small slds-m-top_x-small slds-text-color_warning">[Warning] Data comparison may produce unexpected results with duplicate foreign keys.</p>';
        html += '</div>';
    ...
    generateReport() {
        ...
        console.log('[ComparisonStatus] Navigating to report generator for:', this.comparisonId);
        window.location.href = `/data-comparison/report-generator?comparisonId=${this.comparisonId}`;
    }
