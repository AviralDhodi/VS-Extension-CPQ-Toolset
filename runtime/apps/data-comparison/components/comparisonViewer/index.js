...
class ComparisonViewer {
    ...
    async loadComparisonResults() {
        try {
            ...
            if (errorSection) {
                errorSection.classList.add('slds-hide');
            }
            ...
            if (response.ok) {
                ...
                this.logToShell('info', `Received CSV text length: ${text.length} characters`);
    ...
    parseDiffResults(csvText) {
        try {
            // Use robust CSV parsing that handles multiline values
            const parsed = this.parseFullCSV(csvText);
            
            this.logToShell('info', `Parsed ${parsed.length} rows from CSV`);
            ...
            parsed.forEach((row, rowIndex) => {
                row.forEach((field, fieldIndex) => {
                    if (field && field.length > maxFieldLength) {
                        maxFieldLength = field.length;
                    }
                    // Check if field looks truncated (ends abruptly without closing quote or typical ending)
                    if (field && field.length > 1000 && !field.endsWith('}') && !field.endsWith(';') && !field.endsWith('.')) {
                        truncatedFields++;
                        if (truncatedFields <= 3) {
                            this.logToShell('warning', `Potentially truncated field at row ${rowIndex}, col ${fieldIndex}: ...${field.slice(-50)}`);
                        }
                    }
                });
            });
            this.logToShell('info', `Max field length: ${maxFieldLength} chars, Potentially truncated fields: ${truncatedFields}`);
            ...
            for (let i = 1; i < parsed.length; i++) {
                ...
                if (values && values.length === headers.length) {
                    ...
                    const foreignKey = row.ForeignKeyValue || row.ID || '';
                    const objectField = row.ObjectFieldName || row.Object || '';
                    
                    // Get org values to check for "Both values missing"
                    const [org1Col, org2Col] = this.orgColumns.length >= 2 ? this.orgColumns : ['Org_1', 'Org_2'];
                    const org1Value = row[org1Col] || '';
                    const org2Value = row[org2Col] || '';
                    
                    // Skip rows with invalid data
                    const hasValidForeignKey = foreignKey && foreignKey !== 'N/A' && foreignKey.trim() !== '';
                    const hasValidObjectField = objectField && 
                                               !objectField.includes('Unknown Field') && 
                                               objectField.trim() !== '';
                    const hasBothValuesMissing = (org1Value === 'MISSING' || org1Value === '' || !org1Value) && 
                                               (org2Value === 'MISSING' || org2Value === '' || !org2Value);
                    
                    if (hasValidForeignKey && hasValidObjectField && !hasBothValuesMissing) {
                        data.push(row);
                    }
                }
            }
            ...
            this.diffData = data;
            this.logToShell('info', `Filtered data to ${data.length} valid differences`);
    ...
    renderDiffView() {
            ...
            diffContent.innerHTML = `
                <div class="slds-illustration slds-illustration_large">
                    <div class="slds-text-align_center slds-m-vertical_xx-large">
                        ...
                        <p class="slds-text-body_regular slds-text-color_weak">No comparison results match the current filter.</p>
                    </div>
                </div>
            `;
            return;
        ...
        Object.keys(grouped).sort().forEach(object => {
            html += `<article class="slds-card slds-m-bottom_medium">
                <div class="slds-card__header slds-grid">
                    <header class="slds-media slds-media_center slds-has-flexi-truncate">
                        <div class="slds-media__body">
                            <h2 class="slds-card__header-title">
                                <span class="slds-text-heading_small">${object}</span>
                            </h2>
                        </div>
                    </header>
                </div>
            ...
            grouped[object].forEach(row => {
                ...
                // Debug large values
                if (objectField.includes('SBQQ__') && (org1Value.length > 10000 || org2Value.length > 10000)) {
                    this.logToShell('info', `Large field detected: ${objectField} - Org1: ${org1Value.length} chars, Org2: ${org2Value.length} chars`);
                }
                
                // Extract org names from column names (remove "Org_" prefix)
                const org1Name = org1Col.replace('Org_', '');
                const org2Name = org2Col.replace('Org_', '');
                
                // Generate git-like diff for the values
                const diffHtml = this.generateGitLikeDiff(org1Value, org2Value, org1Name, org2Name);
    ...
    formatValue(value) {
        ...
        // Convert to string and preserve line breaks
        let formatted = String(value);
        
        // Check if value is too large to display
        if (formatted.length > 5000) {
            return 'VALUE TOO BIG TO DISPLAY. DOWNLOAD EXTRACT FOR VALUES.';
        }
        ...
        for (const [encoded, decoded] of Object.entries(decodingMap)) {
            formatted = formatted.replace(new RegExp(encoded, 'g'), decoded);
        }
    ...
    generateGitLikeDiff(oldValue, newValue, oldLabel, newLabel) {
        const truncateMsg = 'VALUE TOO BIG TO DISPLAY. DOWNLOAD EXTRACT FOR VALUES.';
        
        // Handle truncated values
        if (oldValue === truncateMsg || newValue === truncateMsg) {
            return `<div class="diff-line-modified">
                <div class="diff-line-header">--- ${oldLabel}</div>
                <div class="old-value-diff slds-text-color_weak">${oldValue}</div>
                <div class="diff-line-header">+++ ${newLabel}</div>
                <div class="new-value-diff slds-text-color_weak">${newValue}</div>
            </div>`;
        }
    ...
    escapeHtml(text) {
        ...
        // Then properly escape for safe display
    ...
    hideLoadingState() {
        ...
        if (loadingState) {
            loadingState.classList.add('slds-hide');
        }
    }
    ...
    showError(message) {
        ...
        if (errorSection && errorMessage) {
            ...
            errorSection.classList.remove('slds-hide');
        }
