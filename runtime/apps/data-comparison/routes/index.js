...
function serveComponent(componentName, data = {}) {
  return (req, res) => {
    try {
      ...
      // For comparisonViewer, default to slds-index.html if index.html doesn't exist
      ...
      // For configGenerator, default to improved-index.html if index.html doesn't exist
      ...
      let html = pkgReader.readFileSync(htmlPath, 'utf8');
      let css = '';
      let js = '';

      if (pkgReader.existsSync(cssPath)) {
        css = pkgReader.readFileSync(cssPath, 'utf8');
      }

      if (pkgReader.existsSync(jsPath)) {
        js = pkgReader.readFileSync(jsPath, 'utf8');
      }
      ...
      if (componentName === 'configGenerator' && stateManager.shouldUseImproved(componentName)) {
        ...
        // Use regular index.css (improved-index.css doesn't exist)
        const improvedCssPath = pathResolver.getComponentPath('data-comparison', componentName, 'index.css');
        const improvedJsPath = pathResolver.getComponentPath('data-comparison', componentName, 'improved-index.js');
        const fixesCssPath = pathResolver.getComponentPath('data-comparison', componentName, 'slds-fixes.css');

        if (pkgReader.existsSync(improvedHtmlPath)) {
          html = pkgReader.readFileSync(improvedHtmlPath, 'utf8');
          
          // Load fixes CSS first, then component CSS
          if (pkgReader.existsSync(fixesCssPath)) {
            css = pkgReader.readFileSync(fixesCssPath, 'utf8') + '\n';
          }
          if (pkgReader.existsSync(improvedCssPath)) {
            css += pkgReader.readFileSync(improvedCssPath, 'utf8');
          }
          if (pkgReader.existsSync(improvedJsPath)) {
            js = pkgReader.readFileSync(improvedJsPath, 'utf8');
          }
        }
      }
      ...
      if (componentName === 'comparisonViewer' && stateManager.shouldUseImproved(componentName)) {
        ...
        if (pkgReader.existsSync(sldsHtmlPath)) {
          html = pkgReader.readFileSync(sldsHtmlPath, 'utf8');
          
          if (pkgReader.existsSync(sldsCssPath)) {
            css = pkgReader.readFileSync(sldsCssPath, 'utf8');
          }
      ...
      // Create proper SLDS page structure
      const fullHtml = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${componentName} - Data Comparison</title>
...
// Component static file serving for pkg compatibility
router.get('/components/:component/:file', (req, res, next) => {
  const { component, file } = req.params;
  const componentDir = path.join(pathResolver.getAppPath('data-comparison'), 'components', component);
...
router.post('/api/objects/:objectName/fields', async (req, res) => {
  try {
    ...
    // Handle both old format (string) and new format (object with username/alias)
    const firstOrgAlias = typeof orgs[0] === 'string' ? orgs[0] : orgs[0].alias;
    const fields = await sfdxRunner.getObjectFields(objectName, firstOrgAlias);
...
router.post('/api/config/upload', upload.single('configFile'), async (req, res) => {
  try {
    ...
    if (!validation.valid) {
      try { fs.unlinkSync(filePath); } catch (e) { /* Ignore cleanup errors in pkg mode */ } // Clean up
    ...
    if (!pkgReader.existsSync(configDir)) {
      pkgReader.mkdirSync(configDir, { recursive: true });
    }
    
    pkgReader.writeFileSync(configPath, JSON.stringify(config, null, 2));
    try { fs.unlinkSync(filePath); } catch (e) { /* Ignore cleanup errors in pkg mode */ } // Clean up upload
...
router.post('/api/comparison/start', async (req, res) => {
  try {
    ...
    if (!pkgReader.existsSync(actualConfigPath)) {
      return res.status(404).json({ error: 'Configuration not found' });
    }

    config = JSON.parse(pkgReader.readFileSync(actualConfigPath, 'utf8'));
...
router.get('/api/comparison/status/:id', (req, res) => {
  ...
  res.json({
    ...
    config: comparison.config || null, // Include configuration data for summary
    recordCount: comparison.recordCount || 0
  });
});

// Get comparison summary for report generation
router.get('/api/comparison/:id/summary', async (req, res) => {
  const { id } = req.params;
  ...
  try {
    // Load the comparison summary from the results
    const summaryPath = pathResolver.getStoragePath('data-comparison', 'data-extract', id, 'comparison_results', 'comparison_summary.json');
    
    if (!pkgReader.existsSync(summaryPath)) {
      return res.status(404).json({ error: 'Comparison summary not found' });
    }

    const summary = JSON.parse(pkgReader.readFileSync(summaryPath, 'utf8'));
    ...
    res.json({
      ...
      objectsProcessed: summary.objects_processed || []
    });
  } catch (error) {
    logger.error('Error loading comparison summary:', error);
    res.status(500).json({ error: 'Failed to load comparison summary' });
  }
});
...
// Validate SOQL fields
router.post('/api/validate-soql-fields', async (req, res) => {
    const { orgs, orgUsernames, queries } = req.body;
    
    try {
        const errors = [];
        const validationPromises = [];
        
        // For each org and query, validate fields exist
        for (const orgAlias of orgs) {
            const query = queries[orgAlias];
            if (!query) continue;
            
            const username = orgUsernames[orgAlias] || orgAlias;
            
            validationPromises.push(
                sfdxRunner.describeObject(orgAlias, query.objectName)
                    .then(describe => {
                        ...
                        for (const field of query.fields) {
                            if (!availableFields.has(field)) {
                                errors.push(`${username}: Field '${field}' does not exist on object '${query.objectName}'`);
                            }
                        }
                    })
                    .catch(err => {
                        errors.push(`${username}: Unable to describe object '${query.objectName}' - ${err.message}`);
                    })
            );
        }
        ...
        logger.error('SOQL field validation error:', error);
        res.status(500).json({ 
            valid: false, 
...
router.get('/api/comparison/:id/results', (req, res) => {
  ...
  try {
    const content = pkgReader.readFileSync(comparison.resultPath, 'utf8');
...
router.get('/api/comparison/:id/duplicates', (req, res) => {
  ...
  try {
    const report = JSON.parse(pkgReader.readFileSync(comparison.duplicateReportPath, 'utf8'));
...
router.post('/api/comparison/resolve-duplicates', async (req, res) => {
  ...
  try {
    ...
    pkgReader.writeFileSync(resolutionFile, JSON.stringify(resolutionDict, null, 2));
...
async function continueComparisonAfterResolution(comparisonId) {
  ...
  try {
    ...
    if (comparison.configPath) {
      ...
      if (!pkgReader.existsSync(dataConfigPath)) {
        pkgReader.copyFileSync(comparison.configPath, dataConfigPath);
    ...
    if (pkgReader.existsSync(sourceResultPath)) {
      // Fix CSV headers to show actual org usernames instead of Org_1, Org_2
      try {
        const fixHeadersPath = path.join(pathResolver.getAppPath('data-comparison'), 'scripts', 'fix-csv-headers.js');
        const { fixCSVHeaders } = require(fixHeadersPath);
        fixCSVHeaders(sourceResultPath, configPath);
        logger.info('Fixed CSV headers to show org usernames');
      } catch (error) {
        logger.warn('Failed to fix CSV headers:', error.message);
      }
  ...
  try {
    ...
    const progressCallback = (progressData) => {
      if (progressData && progressData.percentage !== undefined) {
        comparison.phases.dataFetch.progress = Math.min(progressData.percentage, 99); // Keep at 99% until complete
        comparison.progress = Math.floor(30 * (comparison.phases.dataFetch.progress / 100)); // Data fetch is 30% of total
        
        if (progressData.currentObject) {
          comparison.phases.dataFetch.currentObject = progressData.currentObject;
        }
        
        logger.info(`Data fetch progress: ${comparison.phases.dataFetch.progress}% for comparison ${comparisonId}`);
      }
    };
    ...
    if (duplicateResult.exitCode === 1) {
      ...
      if (pkgReader.existsSync(reportPath)) {
        ...
        // v3.1.18 - Only require resolution if there are actually duplicates
    ...
    const dataConfigPath = path.join(dataDir, configFileName.replace('cpq-config-', 'config_'));
    pkgReader.copyFileSync(configPath, dataConfigPath);
    ...
    if (pkgReader.existsSync(sourceResultPath)) {
      ...
      pkgReader.copyFileSync(sourceResultPath, destPath);
...
async function parseCSVConfig(filePath) {
  // For pkg compatibility, use the callback version
  const content = pkgReader.readFileSync(filePath, 'utf8');
  ...
  return new Promise((resolve, reject) => {
    csvParse(content, { columns: true }, (err, records) => {
      if (err) {
        ...
        resolve(transformCSVToConfig(records));
      }
    });
  });
}
...
const reportRoutes = require('./report-generator');
