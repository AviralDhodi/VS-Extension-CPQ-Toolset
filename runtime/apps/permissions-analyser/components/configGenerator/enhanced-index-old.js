...
function renderMetadataTable(type) {
    ...
    configState.selectedOrgs.forEach(org => {
        ...
        orgMetadata.forEach(item => {
            ...
            allMetadata.get(item.fullName).counts[org] = item.assignmentCount || 0;
            orgCounts[org][item.fullName] = item.assignmentCount || 0;
        });
    });
    
    // Sort by total assignment count
            ...
            <table class="slds-table slds-table_cell-buffer slds-table_bordered slds-table_striped">
                        ...
                        ${configState.selectedOrgs.map(org => `
                            <th scope="col" class="slds-text-align_center">
                                <div class="slds-truncate" title="${org}">
                                    ${org.split('@')[0]}
                                    <div class="slds-text-body_small slds-text-color_weak">Assignments</div>
                                </div>
                            </th>
                        `).join('')}
                        <th scope="col" class="slds-text-align_center">
                ...
                <tbody>
                    ${sortedMetadata.map((item, index) => {
                        ...
                        const existsInAllOrgs = configState.selectedOrgs.every(org => item.counts[org] !== undefined);
                            ...
                            <tr class="slds-hint-parent ${!existsInAllOrgs ? 'slds-theme_warning' : ''}">
                                ...
                                ${configState.selectedOrgs.map(org => `
                                    <td class="slds-text-align_center">
                                        ${item.counts[org] !== undefined ? 
                                            `<span class="slds-badge ${item.counts[org] > 0 ? 'slds-badge_lightest' : ''}">${item.counts[org]}</span>` 
                                            : '<span class="slds-text-color_weak">-</span>'}
                                    </td>
                                `).join('')}
                                <td class="slds-text-align_center">
                                    <span class="slds-badge slds-theme_info">${total}</span>
                                </td>
                            </tr>
                        `;
                    }).join('')}
...
function renderConfigReview() {
    ...
    const html = `
        ...
        ${configState.objectSelection === 'SPECIFIC' && configState.specificObjects.length > 0 ? `
            <div class="slds-m-bottom_medium">
                <h3 class="slds-text-heading_small slds-m-bottom_small">Specific Objects</h3>
                <div class="slds-box slds-theme_shade">
