...
class EnhancedPermissionsComparison:
    ...
    def load_org_data(self) -> None:
        ...
        logger.info(f"Found {len(org_dirs)} org directories: {[d.name for d in org_dirs]}")
    ...
    def compare_all(self) -> None:
        ...
        logger.info(f"Loaded data for {len(self.org_data)} organizations: {list(self.org_data.keys())}")
        
        # Compare each permission type if selected
        # Check permissionTypes array for which types to compare
        permission_types = self.config.get('permissionTypes', [])
        permissions = self.config.get('permissions', {})  # This contains the selected metadata items
        logger.info(f"Permission types to compare: {permission_types}")
        logger.info(f"Selected metadata items: {json.dumps(permissions, indent=2)}")
        
        if 'Profile' in permission_types:
            logger.info(f"Comparing profiles (selected items: {permissions.get('Profile', [])}")
            self.comparison_results['details']['profiles'] = self.compare_profiles()
        else:
            logger.info("Profile comparison not enabled")
        
        if 'PermissionSet' in permission_types:
            logger.info(f"Comparing permission sets (selected items: {permissions.get('PermissionSet', [])}")
            self.comparison_results['details']['permissionSets'] = self.compare_permission_sets()
        else:
            logger.info("Permission set comparison not enabled")
        
        if 'PermissionSetGroup' in permission_types:
            logger.info(f"Comparing permission set groups (selected items: {permissions.get('PermissionSetGroup', [])}")
            self.comparison_results['details']['permissionSetGroups'] = self.compare_permission_set_groups()
        else:
            logger.info("Permission set group comparison not enabled")
        
        if 'MutingPermissionSet' in permission_types:
            logger.info(f"Comparing muting permission sets (selected items: {permissions.get('MutingPermissionSet', [])}")
            self.comparison_results['details']['mutingPermissionSets'] = self.compare_muting_permission_sets()
        else:
            logger.info("Muting permission set comparison not enabled")
    ...
    def compare_profiles(self) -> Dict[str, Any]:
        ...
        for org, data in self.org_data.items():
            ...
            logger.info(f"Organization {org} has {len(org_profiles)} profiles: {list(org_profiles)[:5]}...")
        ...
        logger.info(f"Total unique profiles across all orgs: {len(profile_comparison['all_profiles'])}")
        ...
        if profile_comparison['org_profiles']:
            ...
            logger.info(f"Common profiles found in all orgs: {len(profile_comparison['common_profiles'])}: {profile_comparison['common_profiles']}")
        ...
        for org, profiles in profile_comparison['org_profiles'].items():
            ...
            if unique:
                ...
                logger.info(f"Unique profiles in {org}: {len(unique)}")
        ...
        logger.info(f"Selected comparison options for profiles: {selected_options}")
        
        differences_found = 0
        for profile in profile_comparison['common_profiles']:
            logger.debug(f"Comparing permissions for profile: {profile}")
            ...
            if differences:
                ...
                differences_found += 1
                logger.info(f"Found differences in profile {profile}")
        
        logger.info(f"Profile comparison complete. Found differences in {differences_found} out of {len(profile_comparison['common_profiles'])} common profiles")
    ...
    def _compare_object_permissions(self, org_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for org, data in org_data.items():
            obj_perms = data.get('objectPermissions', [])
            logger.debug(f"Org {org} has {len(obj_perms)} object permissions")
            for obj_perm in obj_perms:
                all_objects.add(obj_perm['object'])
        
        logger.info(f"Found {len(all_objects)} unique objects with permissions across all orgs")
        ...
        for obj in sorted(all_objects):
            ...
            if self._has_permission_differences(obj_differences):
                ...
                logger.debug(f"Found differences for object {obj}")
        
        logger.info(f"Object permission comparison complete: {len(differences)} objects have differences")
        return differences
    ...
    def compare_permission_sets(self) -> Dict[str, Any]:
        ...
        for org, data in self.org_data.items():
            ...
            logger.info(f"Organization {org} has {len(org_ps)} permission sets: {list(org_ps)[:5]}...")
        ...
        logger.info(f"Total unique permission sets across all orgs: {len(ps_comparison['all_permission_sets'])}")
        ...
        if ps_comparison['org_permission_sets']:
            ...
            logger.info(f"Common permission sets found in all orgs: {len(ps_comparison['common_permission_sets'])}: {ps_comparison['common_permission_sets']}")
        ...
        for org, psets in ps_comparison['org_permission_sets'].items():
            ...
            if unique:
                ...
                logger.info(f"Unique permission sets in {org}: {len(unique)}")
        ...
        logger.info(f"Selected comparison options for permission sets: {selected_options}")
        
        differences_found = 0
        for ps in ps_comparison['common_permission_sets']:
            logger.debug(f"Comparing permissions for permission set: {ps}")
            ...
            if differences:
                ...
                differences_found += 1
                logger.info(f"Found differences in permission set {ps}")
        
        logger.info(f"Permission set comparison complete. Found differences in {differences_found} out of {len(ps_comparison['common_permission_sets'])} common permission sets")
    ...
    def _generate_summary(self) -> None:
        ...
        logger.info(f"Comparison results details keys: {list(self.comparison_results['details'].keys())}")
        ...
        if 'profiles' in self.comparison_results['details']:
            ...
            logger.info(f"Profile stats: {summary['comparison_stats']['profiles']}")
        else:
            logger.info("No profile data in comparison results")
        ...
        if 'permissionSets' in self.comparison_results['details']:
            ...
            logger.info(f"Permission set stats: {summary['comparison_stats']['permissionSets']}")
        else:
            logger.info("No permission set data in comparison results")
        ...
        if 'permissionSetGroups' in self.comparison_results['details']:
            ...
            logger.info(f"Permission set group stats: {summary['comparison_stats']['permissionSetGroups']}")
        else:
            logger.info("No permission set group data in comparison results")
        
        # Muting permission set stats
        if 'mutingPermissionSets' in self.comparison_results['details']:
            mps_data = self.comparison_results['details']['mutingPermissionSets']
            summary['comparison_stats']['mutingPermissionSets'] = {
                'total': len(mps_data.get('all_muting_permission_sets', [])),
                'common': len(mps_data.get('common_muting_permission_sets', [])),
                'with_differences': len(mps_data.get('muting_permission_set_differences', {}))
            }
            logger.info(f"Muting permission set stats: {summary['comparison_stats']['mutingPermissionSets']}")
        else:
            logger.info("No muting permission set data in comparison results")
    ...
    def _save_results(self) -> None:
        ...
        logger.info(f"Saving comparison results to {self.output_path}")
        logger.info(f"Results structure: {list(self.comparison_results.keys())}")
        logger.info(f"Summary: {json.dumps(self.comparison_results['summary'], indent=2)}")
        
        # Save results
        with open(self.output_path, 'w') as f:
            json.dump(self.comparison_results, f, indent=2)
        
        logger.info(f"Results saved successfully to {self.output_path}")
