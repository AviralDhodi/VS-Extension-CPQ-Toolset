...
async function buildExtension() {
  try {
    ...
    // Step 5: Bundle Node.js for Windows if available
    console.log('📋 Bundling Node.js for Windows...');
    const nodeWinPath = '/Users/aviraldhodi/Downloads/node-v22.17.1-win-x64';
    const nodeWinDest = path.join(distDir, 'node-win64');
    
    if (await fs.pathExists(nodeWinPath)) {
      console.log('  - Copying Windows Node.js...');
      await fs.ensureDir(nodeWinDest);
      await fs.copy(path.join(nodeWinPath, 'node.exe'), path.join(nodeWinDest, 'node.exe'));
      
      // Create Windows launchers
      console.log('  - Creating Windows launcher scripts...');
      await require('./bundle-node-windows.js');
      console.log('  ✅ Windows Node.js bundled successfully');
    } else {
      console.log('  ℹ️  Windows Node.js not found - skipping bundling');
    }
    
    // Step 6: Update extension.js to use bundled server
    ...
    // Step 7: Create .vscodeignore if it doesn't exist
